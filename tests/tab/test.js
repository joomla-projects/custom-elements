...
const url = 'http://localhost:3000/tests/tab/tab.html';

test.describe('<joomla-alert-levels>', () => {

  test('Custom Element script is loaded', async ({ page }) => {
    await page.goto(url);
    await expect(page.locator('joomla-tab')).toBeDefined();
    await expect(page.locator('joomla-tab-element')).toBeDefined();
  });

  test('Has type view', async ({ page }) => {
    await page.goto(url);
    await expect(page.locator('joomla-tab')).toHaveAttribute('view', 'tabs');
  });
  ...
  test('Change to Accordion', async ({ page }) => {
    await page.goto(url);
    const element = await page.locator('joomla-tab');
    await element.evaluate(node => node.setAttribute('view', 'accordion'))
    const navEl = await page.locator('[role="tablist"]');
    const view = await element.evaluate(node => node.getAttribute('view'))
    const isHidden = await navEl.evaluate(node => node.hasAttribute('hidden'))
    const nav = await navEl.evaluate(node => node instanceof HTMLElement)
