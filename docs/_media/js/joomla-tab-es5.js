...
class TabsElement extends HTMLElement {
  ...
  keyBehaviour(e) {
    ...
    if (![...this.tabs.map(el => el.tabButton), ...this.tabs.map(el => el.accordionButton)].includes(document.activeElement) || e.metaKey || e.altKey) {
      return;
    }
    let previousTabItem;
    let nextTabItem;
    if (this.view === 'tabs') {
      ...
      const currentTabIndex = this.tabs.map(el => el.accordionButton).findIndex(tab => tab === document.activeElement);
      previousTabItem = currentTabIndex - 1 >= 0 ? this.tabs[currentTabIndex - 1] : this.tabs[this.tabs.length - 1];
      nextTabItem = currentTabIndex + 1 <= this.tabs.length - 1 ? this.tabs[currentTabIndex + 1] : this.tabs[0];
    }
  ...
  removeNavs(tab) {
    ...
    const tabButton = this.tabButtonContainer.querySelector("[aria-controls=".concat(accordionButton.id, "]"));
    if (tabButton) {
  ...
  activateFromState() {
    ...
    if (href) {
      ...
      if (currentTabIndex >= 0) {
        ...
        if (childTabs) {
          const activeTabs = [].slice.call(this.querySelectorAll('joomla-tab-element')).reverse().filter(activeTabEl => activeTabEl.id === href);
          if (activeTabs.length) {
            // Activate the deepest tab
            let activeTab = activeTabs[0].closest('joomla-tab');
            [].slice.call(activeTab.querySelectorAll('joomla-tab-element')).forEach(tabEl => {
            ...
            [].slice.call(this.children).filter(el => el.tagName.toLowerCase() === 'joomla-tab-element').forEach(tabEl => {
              tabEl.removeAttribute('active');
